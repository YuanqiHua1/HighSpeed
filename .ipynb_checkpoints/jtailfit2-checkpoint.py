...
def skeleton_longest_path_from_start(sk255, start_xy,dir_xy=None):
    ...
    while q:
        ...
        if dir_xy is None:
            ...
            dn = float(np.hypot(dv[0], dv[1])) + 1e-6
            dv /= dn
...
def tailfit_skeleton(
    ...
    for framenum, frame in enumerate(vid):
        ...
        # 3) skeletonize（只对尾巴 bbox 做，巨幅提速）
        ys, xs = np.where(bw > 0)
        ...
        if poly is None:
            ...
            if enable_endpoint_gate:
                ...
                if poly_end_y > end_y + float(endpoint_y_margin):
                    # 很典型：被下面那根黑色竖条/黑背景带走
                    use_fallback = True
                    qc_endpoint_gate += 1  # 记到 short 里（你也可以单独计数）
                    qc_short_skel_frames += 1
    ...
    if qc_fallback_frames > 0 or qc_short_skel_frames > 0:
        print(
            f"————————————————————————————————————————————————————\n"
